		...
		if (card && Firebase.ready() && (millis() - dataMillis > 60000 || dataMillis == 0))
		{
			...
			if (!taskCompleted)
			{
				...
				if (Firebase.Firestore.getDocument(&fbdo, FIREBASE_PROJECT_ID, "", buffer)) {
					...
					// Fetch value
					const char* perm_level = doc["fields"]["perm_level"]["stringValue"];
					Serial.printf("perm_level   \n%s\n\n", perm_level);
					...
					//if (Firebase.Firestore.runQuery(&fbdo, FIREBASE_PROJECT_ID, "", "Students/", &query))
					//	Serial.printf("ok\n%s\n\n", fbdo.payload().c_str());
					//else
					...
					//current machine permission is 1
					
					//FirebaseJson query;
					//query.set("select/fields/[0]/fieldPath", "name");
					//query.set("select/fields/[1]/fieldPath", "perm_level");
					//query.set("from/collectionId", tmpStr);
				...
				//
				//	if (Firebase.Firestore.createDocument(&fbdo, FIREBASE_PROJECT_ID, "" /* databaseId can be (default) or empty */, buffer2, content.raw()))
				//		Serial.printf("ok\n%s\n\n", fbdo.payload().c_str());
				//	else
				//		Serial.println(fbdo.errorReason());
				}
